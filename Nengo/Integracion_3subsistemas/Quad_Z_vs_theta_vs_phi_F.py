...
with model:
    ...
    Phi_aux = nengo.Ensemble(n_neurons=200, dimensions=2, radius=Sphi)
    ...
    En_ref_phi=nengo.Ensemble(n_neurons=200, dimensions=1,radius=Sphi)
    ...
    def RetroPhi_Y(x):
        u= x[0]
        Theta = x[1] 
        return g*tan(u) / cos(Theta)
    nengo.Connection(Phi_aux, U_phi, function=RetroPhi_Y, synapse=t_syn)
    nengo.Connection(U_phi,Gy,function=None,synapse=None)
    #################################################################################
    ...
    plot_X(sim.data, num_xticks=6, num_yticks=6, xmin=-1, xmax=EjeX, ymin=-5, ymax=5)
    ...
    plot_Y(sim.data, num_xticks=6, num_yticks=6, xmin=-1, xmax=EjeX, ymin=-5, ymax=5)
    ...
    plot_Z(sim.data, num_xticks=6, num_yticks=6, xmin=-1, xmax=EjeX, ymin=-10, ymax=10)
    ...
    def Plot3D_XYZ(data):
        ...
        ZValores = data[Fx_p][:, 0]
        XValores = data[Fy_p][:,0]
        YValores = data[Fz_p][:,0]
        ...
        plt.savefig('GraficaXYZ', format="pdf", bbox_inches='tight')
        plt.show()  
    Plot3D_XYZ(sim.data)