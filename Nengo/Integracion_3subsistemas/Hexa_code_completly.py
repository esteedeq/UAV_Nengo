...
with model:
    ...
    def upsilon_x(x): 
        return -g*tan(x)
    nengo.Connection(At[0],U_x,function=upsilon_x) # 
    ...
    CC1 = nengo.Ensemble(n_neurons=500, dimensions=2, radius=Sphi)
    nengo.Connection(U_y, CC1[0], synapse=t_syn)
    nengo.Connection(At[0], CC1[1], synapse=t_syn)
    

    def phi_d(x):
        u= x[0]
        Theta = x[1] 
        return arctan2(u*cos(Theta),g) 
    ...
    nengo.Connection(CC1,En_ref_phi,function=phi_d,synapse=t_syn)
    ...
    nengo.Connection(Retro_auxiliar, U_y, function=Upsilon_y, synapse=t_syn)
    ...
    def gz_fun(x):
        return [0,x[0]*t_syn]
    def upsilon_z(x):
        return (-x[0]*cos(x[1])*cos(x[2]))/m +g #
    ...
    ########### Eq. (6)
    def u_fun(x): 
        return (x[0]+g)*m/(cos(x[1])*cos(x[2]))#(x[0]+g)*m/(cos(0)*cos(0))
    nengo.Connection(u,CC[0],function=u_fun,synapse=None)
####################################################################################################################################

    # "Control de Posicion psi"
    # F_psi=nengo.Ensemble(n_neurons=500,dimensions=2,radius=Sx)
    # G_psi=nengo.Ensemble(n_neurons=200,dimensions=1,radius=Sx)
    ...
    # #ref_x=nengo.Node(lambda x, k=1, centro=5: 1 / (1 + np.exp(-k * (x - centro))))
    # ref_psi = nengo.Node(lambda t: 0.1 if t < 0.01 else 0)
    # En_ref_psi=nengo.Ensemble(n_neurons=200,dimensions=1,radius=Sx,neuron_type=nengo.Direct())
    # nengo.Connection(ref_psi,En_ref_psi)
    
    # #Errores en psi
    # Err_psi=nengo.Ensemble(n_neurons=200,dimensions=1,radius=Sx)
    # nengo.Connection(F_psi[0],Err_psi,synapse=E_syn, transform=-1)
    # nengo.Connection(En_ref_psi,Err_psi,transform=1,synapse=E_syn)
    # #Derivada del error en  psi 
    # D_err_psi=nengo.Ensemble(n_neurons=200,dimensions=1,radius=Sx)
    # nengo.Connection(Err_psi,D_err_psi,synapse=E_syn,transform=20)
    # nengo.Connection(Err_psi,D_err_psi,synapse=t_syn,transform=-20)

    # #Integral del error en psi
    # In_Err_psi = nengo.Ensemble(n_neurons=500, dimensions=1, radius=Sx)
    # nengo.Connection(Err_psi, In_Err_psi, transform= kix, synapse=E_syn)
    # nengo.Connection(In_Err_psi, In_Err_psi,transform=1, synapse=E_syn)
    

    # #Control, para la obtencion de u_psi
    # u_psi=nengo.Ensemble(n_neurons=500,dimensions=1,radius=Sx*2)
    # nengo.Connection(Err_psi,u_psi,transform=kpx)
    # nengo.Connection(D_err_psi,u_psi,transform=kdx)
    # nengo.Connection(In_Err_psi,u_psi,transform=1)
...
####################################################################################################################################
    ...
    U_p=nengo.Probe(CC[0],synapse=t_syn)
