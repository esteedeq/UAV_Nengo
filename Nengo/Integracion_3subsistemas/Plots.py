...
datosPsi = pd.read_csv('simulink_data_psi.csv', skiprows=1, header=None)
...
datosInputPsi = pd.read_csv('simulink_data_TauPsi.csv', skiprows=1, header=None)
...
# Datos Psi
tiempo_Psi = datosPsi[0].values
PsiValores = datosPsi[1].values
PsiDotValores = datosPsi[2].values
...
# Datos In_Psi
tiempo_InputPsi = datosInputPsi[0].values
InputValoresPsi = datosInputPsi[1].values
...
# ============================================
fig2, (ax4, ax5, ax6) = plt.subplots(3, 1, figsize=(12, 8))
...
# Gráfica Psi
ax6.plot(tiempo_Psi, PsiValores, linewidth=Grosor_linea, label=r'$\psi$')
ax6.set_xlabel('Time [s]', fontsize=28)
ax6.set_ylabel('Angle [rad]', fontsize=28)
...
# ============================================
fig3, (ax6, ax7, ax8, ax9) = plt.subplots(4, 1, figsize=(12, 8))
...
# Gráfica Tau Phi
ax9.plot(tiempo_InputPsi, InputValoresPsi, linewidth=Grosor_linea, label=r'$\tau_\psi$')
ax9.set_xlabel('Time [s]', fontsize=28)
ax9.set_ylabel('M [Nm]', fontsize=28)
ax9.grid(True, linestyle='-.')
ax9.legend(fontsize='28', loc='upper right')
