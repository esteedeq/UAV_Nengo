...
Source model: `Quadri_Victor_combination.slx`.

Purpose: convert the combined Victor quadrotor PD controller into a combined cascaded PID controller.

Generated Simulink model:

```text
models/Quadri_Victor_combination_PID_R2025a.slx
```

This model uses the same equations as the separated PID model, but the controller is documented and diagrammed as one combined cascaded PID controller: position PID generates desired attitude and thrust, then attitude PID generates body moment commands.

## Reference Mission

The original Victor model uses the same sigmoid channel missions as `Quadri_Sub_Por_Partes`, plus Nengo-generated `sim_data_*` traces for comparison. The PID benchmark reproduces the deterministic reference missions:
